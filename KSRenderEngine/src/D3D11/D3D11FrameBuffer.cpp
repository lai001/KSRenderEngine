...
namespace ks
{
		...
		:engineInfo(engineInfo), clearColor(glm::vec4(0.0))
	{
		...
		readTexture2D = D3D11Texture2D::createReadBackTexture2D(width, height, TextureFormat::R8G8B8A8_UNORM, engineInfo);
	...
	void D3D11FrameBuffer::bind() const
	{
		...
		d3dDeviceContext->ClearRenderTargetView(view, glm::value_ptr(clearColor));
